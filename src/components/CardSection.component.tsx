...
interface CardSectionProps<T extends { id?: number; name?: string; backdrop_path?: string | null }> {
  title?: string;
  data?: T[];
  children?: React.ReactNode;
}

export const CardSectionComponent = <T extends { id?: number; name?: string; backdrop_path?: string | null },>({ title, data }: CardSectionProps<T>) => {
  //  console.log('p>>',data)
  ...
  useEffect(() => {
    ...
    const movieId = svgEle?.dataset?.movieId
    if (svgEle && movieId) {
  ...
  const handleWatchList = (e: React.MouseEvent<SVGElement>) => {
    ...
    if (e.currentTarget instanceof SVGElement) {
      ...
      addMovieToWatchList({
        dataObj: {
          id: findedData?.id ?? undefined,
          backdrop_path: findedData?.backdrop_path ?? undefined,
          name: findedData?.name ?? undefined,
          //    release_date:
          // or
          // first_air_date:
        },
      ...
      <CardSection>
        ...
        <div className="card-scrolling-div">
          {data?.map((cu) => (
            ...
            >
              <img
                src={`${import.meta.env.VITE_TMDB_IMAGE_BASE_URL}/w500${cu?.backdrop_path
                  }`}
                alt="img"
              />
