...
export const MovieDetailsComponent = () => {
  ...
  const handleWatchList = (e: React.MouseEvent<SVGElement>) => {
    ...
    void datatype; // suppress unused warning - kept for future use

    // find the movie using id

    addMovieToWatchList({
      dataObj: {
        id: fetchData?.id ?? undefined,
  ...
  async function handleRelatedMovieData() {
    try {
      setIsLoading(true)
      const res = await fetch(`${import.meta.env.VITE_TMDB_API_URL}/movie/${(state as { movieId: string | number }).movieId}/recommendations?language=en-US&page=1&api_key=${import.meta.env.VITE_TMDB_KEY}`)
  ...
  useEffect(() => {
    ...
    console.log('o>', (fetchData as unknown as Record<string, unknown>)?.['success'])
    if ((fetchData as unknown as Record<string, unknown>)?.['success'] === false) {
